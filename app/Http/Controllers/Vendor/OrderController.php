...
class OrderController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of(Order::whereIn('id',$orderIds))->addColumn('index', function ($row) {
                ...
                ->addColumn('details', function ($row) use ($rate, $sign) {
                    ...
                    foreach ($items as $item) {
                        if(Product::find($item->id)->user_id==auth()->user()->id){
                ...
                <input data-href="' . URL::to('vendor/order/payment-status/' . $row->id) . '"  type="checkbox"' . $checked . ' class="switch-status switch ts-swich-input" name="status" id="" value="1">
                ...
                <a href="' . route( "vendor_order.show", $row->id) . '"><i class="ri-eye-line"></i></a>
                </span> ';
    ...
    public function show(Order $order)
    {
        ...
        return view('seller.order.show', compact('order', 'items'));
    }
    ...
    {
        $items = unserialize(bzdecompress(utf8_decode($order->cart)));
        return view('seller.order.print', compact('order', 'items'));
    }
