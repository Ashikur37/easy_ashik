...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function __construct()
    {
        ...
        $this->domain=$ps->ssl_mode=="sandbox"?"https://sandbox.sslcommerz.com":"https://securepay.sslcommerz.com";
        $this->setStoreId($ps->store_id);
        $this->setStorePassword($ps->store_password);
    }
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if ($merchant_trans_id != "" && $merchant_trans_amount != 0) {
            ...
            if ($this->SSLCOMMERZ_hash_verify($post_data, $this->getStorePassword())) {
                ...
                $requested_url = ($this->domain. $this->config['apiUrl']['order_validate'] . "?val_id=" . $val_id . "&store_id=" . $store_id . "&store_passwd=" . $store_passwd . "&v=1&format=json");
...
//                    }
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        $header = [];
        $this->setApiUrl($this->domain . $this->config['apiUrl']['make_payment']);
