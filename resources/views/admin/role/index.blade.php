@extends('layouts.admin',['headerText' => $lng->RoleList])
@section('title', "$lng->RoleList")
@section('style')
            ...
            <div class="col-12">
                <div class="flex-item top-info-header">
                    <div class="d-flex">
                        <div>
                            @can('role.create')
                                <a href="{{ route('role.create') }}" class="submit-btn mr-3">
                                    {{ $lng->Add }}
                                </a>
                            @endcan
                        </div>
                        <div class="action-wrapper">
                            @can('role.destroy')
                                <button class="trash-btn delete-button-head mr-3"
                                    data-route="{{ URL::to('/admin/role/multi') }}">
                                    {{ $lng->Delete }}
                                </button>
                            @endcan
                        </div>
...
@section('script')
    @include('includes.scripts.admin.data-table', ['route' => 'role.index','columns'=>[
    auth()->user()->can('role.edit')||auth()->user()->can('role.destroy')?
    [
    "name"=>'index',
    'order'=>'false'
    ]:null,
