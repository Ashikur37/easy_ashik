@extends('layouts.admin',['headerText' => "$lng->Edit"." "."$lng->SubCategory"])
@section('title', "$lng->Edit" . ' ' . "$lng->SubCategory")
@section('style')
...
@section('breadcrumb')
    <li class="breadcrumb-item">
        ...
        <a href="#">{{ $lng->Edit . ' ' . $lng->SubCategory }}</a>
    </li>
@endsection
@section('content')
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input value="{{ $subCategory->name }}" name="name" required type="text" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" class="select2 form-control">
                            @foreach ($categories as $category)
                                <option @if ($subCategory->category_id == $category->id) selected @endif value="{{ $category->id }}">{{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="status" class="select2 form-control">
                            <option {{ $subCategory->status == 1 ? 'selected' : '' }} value="1">Enabled</option>
                            <option {{ $subCategory->status == 0 ? 'selected' : '' }} value="0">{{ $lng->Disabled }}
                            </option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>{{ $lng->URL }} </label>
                        <input value="{{ $subCategory->slug }}" name="slug" type="text" class="form-control"
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input value="{{ $subCategory->image }}" type="hidden" name="image" id="image">
                        @error('image')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label>{{ $lng->SubCategory }} {{ $lng->Banner }} </label>
                        <div id="bannerUpload" class="dropzone">
                        </div>
                        <input value="{{ $subCategory->banner }}" type="hidden" name="banner" id="banner">
                    </div>
                </div>
            </div>
        </form>
...
@section('script')
    ...
    <script>
        ...
        $(function() {
            ...
            imageDropzone.createThumbnailFromUrl(imageFile,
                "{{ URL::to('/') }}/images/category/{{ $subCategory->image }}");
            setTimeout(function() {
                $(".dz-remove").html('<i class="ri-close-line"></i>')
                $(".dz-details").html("")
            }, 500)
            ...
            let bannerFile = { name: "{{$subCategory->banner}}" };
          bannerDropzone.emit("addedfile", bannerFile);
          bannerDropzone.createThumbnailFromUrl(bannerFile, "{{URL::to('/')}}/images/category/{{$subCategory->banner}}");
          setTimeout(function(){
                    $(".dz-remove").html('<i class="ri-close-line"></i>')
                    $(".dz-details").html("")
                    },500)
