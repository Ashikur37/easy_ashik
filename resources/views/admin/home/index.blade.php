@extends('layouts.admin',['headerText' => $lng->Dashboard])
@section('title', "$lng->AdminDashboard")
@section('style')
    ...
    <div class="mb-20 dashboard-element-wrapper mt-80">
        ...
        <div class="row mt-20 prl-5">
            ...
            <div class="col-xl-6 col-md-6 col-12 prl-10 d-flex align-items-stretch mt-20 order-4 order-xl-5">
                <div class="p-4 dashboard-element">
                    <h5 class="mb-4">{{ $lng->RecentActivity }}</h5>
                                ...
                                <div class="d-flex justify-content-between">
                                    <div class="recent-activity-content">
                                        ...
                                        <div>
                                            ...
                                            <p class="mb-0">{!! Str::limit($notification->data['text'], 150) !!}</p>
                                        </div>
                                    </div>
                                    <div class="recent-activity-timer">
                                        <span
                                            class="time">{{ $notification->created_at->diffForHumans(null, true, true) }}</span>
                                        <span
        ...
        <div class="row mt-20 prl-5">
            <div class="col-xl-6 col-12 prl-10 d-flex align-items-stretch">
                <div class="p-4 dashboard-element">
                    <h5 class="mb-4">{{ $lng->Ticket }}</h5>
                    <div class="filter-option">
                        <span data-toggle="dropdown" class="filter-option-trigger"><i class="ri-more-2-fill"></i></span>
                        <div class="dropdown-menu dropdown-menu-right filter-option-menu">
                            <a href="{{ route('ticket.index') }}" class="dropdown-item">{{ $lng->SeeAll }}</a>
                        </div>
                    </div>
                    <div class="dashboard-table responsive-table ">
                        <table class="table mb-0">
                            <thead>
                                <tr>
                                    <th class="shortest-th">{{ $lng->User }}</th>
                                    <th></th>
                                    <th>{{ $lng->Subject }}</th>
                                    <th>{{ $lng->Type }}</th>
                                    <th>{{ $lng->Message }}</th>
                            ...
                            <tbody>
                                @foreach ($recentTickets as $ticket)
                                    <tr class="dashboard-table-row" data-href="{{ route('ticket.index') }}">
                                        <td>
                                            <div class="text-left user">
                                                <div class="user-img">
                                                    <span class="status-unread"></span>
                                                    <img src="{{ $ticket->user->getImageUrl() }}" alt="User">
                                                </div>
                                            </div>
                                        </td>
                                        <td class="pl-0">
                                        ...
                                        <td><span class="message">{{ $ticket->messages->last()->message }} </span>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        <div class="row mt-20 prl-5">
            <div class="col-xl-6 col-12 prl-10 d-flex align-items-stretch">
                <div class="p-4 dashboard-element">
                    <h5 class="mb-4">{{ $lng->LatestReviews }}</h5>
                    <div class="filter-option">
                        <span data-toggle="dropdown" class="filter-option-trigger"><i class="ri-more-2-fill"></i></span>
                        <div class="dropdown-menu dropdown-menu-right filter-option-menu">
                            <a href="{{ route('review.index') }}" class="dropdown-item">{{ $lng->SeeAll }}</a>
                        </div>
                    </div>
                    <div class="dashboard-table responsive-table ">
                        <table class="table mb-0">
                            <thead>
                                <tr>
                                    <th class="shortest-th pr-0">{{ $lng->Product }}</th>
                                    <th></th>
                                    <th class="medius-th">{{ $lng->User }}</th>
                                    <th class="short-th">{{ $lng->Rating }}</th>
                                    <th>{{ $lng->Comment }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($recentReviews as $review)
                                    <tr class="dashboard-table-row" data-href="{{ route('review.index') }}">
                                        <td>
                                            <div class="text-left product">
                                                <div class="product-img">
                                                    <img src="{{ asset('images/product/' . $review->product->image) }}"
                                                        alt="Product">
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td><span class="comment">{{ Str::limit($review->comment, 40) }}</span></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="col-xl-6 col-12 prl-10 d-flex align-items-stretch mt-20 mt-xl-0">
                <div class="p-4 dashboard-element">
                    <h5 class="mb-4">{{ $lng->LatestComments }}</h5>
                    <div class="filter-option">
                        <span data-toggle="dropdown" class="filter-option-trigger"><i class="ri-more-2-fill"></i></span>
                        <div class="dropdown-menu dropdown-menu-right filter-option-menu">
                            <a href="{{ route('comment.index') }}" class="dropdown-item">{{ $lng->SeeAll }}</a>
                        </div>
                    </div>
                    <div class="dashboard-table responsive-table ">
                        <table class="table mb-0">
                            ...
                            <tbody>
                                @foreach ($recentComments as $comment)
                                    <tr class="dashboard-table-row"
                                        data-href="{{ route('front-product.show', $comment->product->slug) }}">
                                        <td>
                                            <div class="text-left product">
                                                <div class="product-img">
                                                    <img src="{{ asset('images/product/' . $comment->product->image) }}"
                                                        alt="Product">
                                                </div>
                                            </div>
                                        </td>
                                        <td class="pl-0">
                                            <div class="product-details">
                                                <p class="mb-0">{{ Str::limit($comment->product->name, 40) }}</p>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td><span class="comment">{{ Str::limit($comment->text, 40) }} </span></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
