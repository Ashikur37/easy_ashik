@extends('layouts.admin',['headerText' => $lng->BrandList])
@section('title', "$lng->BrandList") 
@section('style')
        ...
        <div class="col-12">
            <div class="flex-item top-info-header">
                <div class="d-flex">
                    <div>
                        @can('brand.create')
                        <a href="{{route('brand.create')}}" class="submit-btn mr-3">
                            {{$lng->Add}}
                        </a>
                        @endcan
                    </div>
                    <div class="action-wrapper">
                        @can('brand.edit')
                        <select class="select2 mr-3 select-status-head" data-route="{{URL::to('/admin/brand/multi')}}">
                        ...
                        @endcan
                        @can('brand.destroy')
                        <button class="trash-btn delete-button-head mr-3" data-route="{{URL::to('/admin/brand/multi')}}">{{$lng->Delete}}</button>
                        @endcan
                    </div>
                </div>               
                <div class="d-flex ">
            ...
            <div class="card card-table">
                <div class="table-responsive">
                    <table class="table table-striped first" id="takwa-table">
                        <thead>
                            <tr>
                            @canany(['brand.edit','brand.destroy'])
                                <th>
...
['route' => 'brand.index','columns'=>[
    auth()->user()->can('brand.edit')||auth()->user()->can('brand.destroy')?
        [
        "name"=>'index',
        'order'=>'false'
        ]:null,
