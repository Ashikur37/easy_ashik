@extends('layouts.vendor',['headerText' => $lng->EditProduct])
...
<form onsubmit="return validateProduct()" method="post" action="{{route('vendor.product_update',$product->id)}}">
    @method('patch')
    @csrf
        <div class="row">
            <div class="col-12">
...
function removeLabelRow(el,id=null) {
    ...
    if(id){
        $.get("{{URL::to('/vendor/remove-option-value')}}/"+id, function(data, status){
  });
    }
}
function removeOption(id){
    document.getElementById('optionRow'+id).remove();
    $.get("{{URL::to('/vendor/remove-option')}}/{{$product->id}}/"+id, function(data, status){
  });
}
